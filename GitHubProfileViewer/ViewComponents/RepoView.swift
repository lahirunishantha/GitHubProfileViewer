...
class RepoView: UIView, RepoProtocol {
    ...
    func loadWithRepository(repo: Repository, repoUser: RepoUser) {
        self.uidLabel.text = repoUser.bio
        self.profileImageView.kf.setImage(with: URL(string: repoUser.avatarUrl))
        
        self.projectLabel.text = repo.name
        self.projectDescLabel.text = repo.desc
        
        self.starCountLabel.text = repo.starCount.description
        self.typeDescLabel.text = repo.primaryLanguage
    }
    ...
    func setupViews(){
        ...
        uidLabel.anchor(top: profileImageView.topAnchor, left: profileImageView.rightAnchor, bottom: nil, right: rightAnchor, center: nil, paddingTop: 10, paddingLeft: 8, paddingBottom: 0,
            paddingRight: 10, width: 0, height: 15)
        
        container.addSubview(projectLabel)
        projectLabel.anchor(top: profileImageView.bottomAnchor, left: container.leftAnchor, bottom: nil, right: rightAnchor, center: nil, paddingTop: 5, paddingLeft: 15, paddingBottom: 0,
            paddingRight: 10, width: 0, height: 20)
        container.addSubview(projectDescLabel)
        projectDescLabel.anchor(top: projectLabel.bottomAnchor, left: container.leftAnchor, bottom: nil, right: rightAnchor, center: nil, paddingTop: 3, paddingLeft: 15, paddingBottom: 0,
            paddingRight: 10, width: 0, height: 20)
        
        // Can be make more modular by creating a common component for this start/primary language Views
